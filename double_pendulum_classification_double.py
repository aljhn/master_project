...
for epoch in range(1, epochs + 1):
    try:
        ...
        with torch.no_grad():
            ...
            F = func1(batch_t[0], batch_y_unconstrained_val[0, :, :])
            ...
            for i in range(1, batch_t.shape[0]):
                F = func1(batch_t[i], batch_y_unconstrained_val[i, :, :])
                F = torch.nn.functional.normalize(F, dim=1)
            ...
            F = func1(batch_t[0], batch_y_constrained_val[0, :, :])
            F = torch.nn.functional.normalize(F, dim=1)
            ...
            for i in range(1, batch_t.shape[0]):
                F = func1(batch_t[i], batch_y_constrained_val[i, :, :])
                F = torch.nn.functional.normalize(F, dim=1)
            ...
            F = func2(batch_t[0], batch_y_unconstrained_val[0, :, :])
            F = torch.nn.functional.normalize(F, dim=1)
            dr = batch_dy_unconstrained_val[0, :, :]
            dr = torch.nn.functional.normalize(dr, dim=1)
            dot_prev = torch.sum(F * dr, dim=1)
            ...
            for i in range(1, batch_t.shape[0]):
                F = func2(batch_t[i], batch_y_unconstrained_val[i, :, :])
                F = torch.nn.functional.normalize(F, dim=1)
                dr = batch_dy_unconstrained_val[i, :, :]
                dr = torch.nn.functional.normalize(dr, dim=1)
                dot_next = torch.sum(F * dr, dim=1)
            ...
            F = func2(batch_t[0], batch_y_constrained_val[0, :, :])
            F = torch.nn.functional.normalize(F, dim=1)
            dr = batch_dy_constrained_val[0, :, :]
            dr = torch.nn.functional.normalize(dr, dim=1)
            dot_prev = torch.sum(F * dr, dim=1)
            ...
            for i in range(1, batch_t.shape[0]):
                F = func2(batch_t[i], batch_y_constrained_val[i, :, :])
                F = torch.nn.functional.normalize(F, dim=1)
                dr = batch_dy_constrained_val[i, :, :]
                dr = torch.nn.functional.normalize(dr, dim=1)
                dot_next = torch.sum(F * dr, dim=1)
            ...
            print("Epoch {:04d} | L2_1 {:.2f} | L2_2 {:.2f} | ULI1 {:.3f} | CLI1 {:.3f} | ULI2 {:.3f} | CLI2 {:.3f} | Classification Accuracy {:2d}%".format(epoch, loss1.item(), loss2.item(), unconstrained_line_integral_loss1.item(), constrained_line_integral_loss1.item(), unconstrained_line_integral_loss2.item(), constrained_line_integral_loss2.item(), accuracy))
            accs[epoch - 1] = accuracy
...
plt.grid()
plt.tight_layout()
plt.savefig("plots/line_integral_doublemodel_acc.pdf")
plt.show()
