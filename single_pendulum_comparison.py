...
with torch.no_grad():
    ...
    #batch_y0_train = torch.randn((batch_size, 2)) * 3
    #batch_y0_val = torch.randn((batch_size, 2)) * 3
...
for epoch in range(1, epochs + 1):
    try:
        ...
        print("Epoch {:04d} | NODE L2 Val {:.4f} | Regressor L2 Val {:.4f} | Time {:.4f}".format(epoch, loss_node_val.item(), loss_regressor_val.item(), timet))
...
y_true = torch.concat((batch_y0_train.unsqueeze(0), batch_y_train), dim=0)

with torch.no_grad():
    y_node = odeint(func_node, batch_y0_train, batch_t)
    y_node = torch.concat((batch_y0_train.unsqueeze(0), y_node), dim=0)

    y_regressor = odeint(func_regressor, batch_y0_train, batch_t)
    y_regressor = torch.concat((batch_y0_train.unsqueeze(0), y_regressor), dim=0)
...
plt.savefig("plots/single_pendulum_trajectories_h01_T1_.pdf")
plt.show()
...
plt.grid()
plt.savefig("plots/single_pendulum_loss_comparison_h01_T1_.pdf")
plt.show()
